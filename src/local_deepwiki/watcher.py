...
class DebouncedHandler(FileSystemEventHandler):
    ...
    async def _do_reindex(self, changed_files: list[str]) -> None:
        ...
        try:
            ...
            console.print(f"[green]Generated {len(wiki_structure.pages)} pages in {wiki_time:.1f}s[/green]")
...
async def initial_index(
    ...
    console.print(f"[green]Indexed {status.total_files} files, {status.total_chunks} chunks[/green]")
