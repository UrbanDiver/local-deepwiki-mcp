...
Covers: TestVectorStoreIndexes, TestVectorStoreSearch, TestVectorStoreStats,
TestVectorStoreAddChunks, TestVectorStoreEdgeCases.
"""
...
class TestVectorStoreIndexes:
    ...
    async def test_delete_chunks_by_files_nonexistent(self, populated_store):
        ...
        result = await populated_store.delete_chunks_by_files(
            ["nonexistent1.py", "nonexistent2.py"]
        )
...
class TestVectorStoreStats:
    ...
    async def test_stats_with_data(self, vector_store):
        ...
        chunks = [
            make_chunk(
                "py_func", language=Language.PYTHON, chunk_type=ChunkType.FUNCTION
            ),
            make_chunk(
                "py_class", language=Language.PYTHON, chunk_type=ChunkType.CLASS
            ),
            make_chunk(
                "ts_func", language=Language.TYPESCRIPT, chunk_type=ChunkType.FUNCTION
            ),
        ]
        await vector_store.create_or_update_table(chunks)
...
class TestVectorStoreEdgeCases:
    ...
    async def test_unicode_content(self, vector_store):
        ...
        chunk = make_chunk(
            "unicode_test",
            content="def hello(): return '\u4f60\u597d\u4e16\u754c \U0001f30d \u041f\u0440\u0438\u0432\u0435\u0442 \u043c\u0438\u0440'",
        )
        await vector_store.create_or_update_table([chunk])
        ...
        assert "\U0001f30d" in result.content
