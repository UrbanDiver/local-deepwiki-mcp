...
This file contains comprehensive test suites for the VectorStore class functionality, including tests for indexing, search operations, statistics, chunk management, and edge cases. The tests use pytest fixtures and mock objects to verify vector store behavior in various scenarios.

## Classes

### MockEmbeddingProvider

A mock implementation of the EmbeddingProvider interface used for testing purposes. This class provides fake embedding functionality to avoid dependencies on actual embedding services during tests.
...
**Key Methods:**
- `vector_store`: Pytest fixture that creates a VectorStore instance for testing using a temporary database path and MockEmbeddingProvider
- `populated_store`: Pytest fixture that creates a vector store with test data, including a chunk from "src/[main](../src/local_deepwiki/web/app.md).py"
...
**Key Methods:**
- `vector_store`: Pytest fixture that creates a VectorStore instance for testing
...
**Key Methods:**
- `vector_store`: Pytest fixture that creates a VectorStore instance for testing
- `test_stats_empty_store`: Tests that statistics for an empty store show zero total chunks and empty languages dictionary
...
**Key Methods:**
- `vector_store`: Pytest fixture that creates a VectorStore instance for testing
- `test_add_to_empty_creates_table`: Verifies that adding chunks to an empty store creates the necessary table structure
...
- `test_create_or_update_empty_list`: Tests that creating/updating with an empty list returns 0 and maintains zero chunk count
...
- `test_delete_file_path_with_quotes`: Tests safe deletion of files with quotes in their paths
- `test_chunk_id_injection_attempt`: Tests protection against SQL-like injection attempts in chunk IDs
- `test_file_path_injection_attempt`: Tests protection against SQL-like injection attempts in file paths
- `test_unicode_content`: Tests handling of Unicode content in chunks
- `test_reopen_database`: Tests database persistence across reopening
- `test_replace_existing_table`: Tests table replacement functionality
- `test_db_path_created_if_not_exists`: Tests automatic database path creation
- `test_single_chunk_operations`: Tests operations on individual chunks
- `test_empty_content_chunk`: Tests handling of chunks with empty content
- `test_large_content_chunk`: Tests handling of chunks with large content
- `test_many_chunks_same_file`: Tests handling of multiple chunks from the same file
- `test_search_limit_zero_raises`: Tests that searching with limit=0 raises a ValueError

## Functions

### make_chunk

A utility function referenced throughout the tests for creating test chunk objects. Based on the usage patterns, it appears to create CodeChunk instances with configurable chunk IDs and file paths.
...
vector_store = VectorStore(db_path, provider)
```

### Testing Empty Store Operations

```python
# Test empty store statistics
...
- **VectorStore**: The [main](../src/local_deepwiki/web/app.md) class being tested from `local_deepwiki.core.vectorstore`
- **EmbeddingProvider**: Base provider class from `local_deepwiki.providers.base`
- **CodeChunk**: Data model from `local_deepwiki.models`
- **ChunkType**: Enumeration from `local_deepwiki.models`
- **Language**: Enumeration from `local_deepwiki.models`

The test suite provides comprehensive coverage of vector store functionality, including normal operations, edge cases, error conditions, and security considerations such as injection attack prevention.
...
- [`tests/test_vectorstore.py:9-28`](https://github.com/UrbanDiver/local-deepwiki-mcp/blob/main/tests/test_vectorstore.py#L9-L28)
